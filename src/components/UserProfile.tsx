...
export function UserProfile() {
    ...
    const avatar = auth.currentUser?.photoURL || '/images/avatar.jpeg';
    const [userImgList, setUserImgList] = useState<any[]>([]);
        ...
        <div>
            <div className='d-flex align-items-center justify-content-between'>
                <div className='d-flex align-items-center'>
                    <img src={avatar} alt='avatar' style={{ width: '50px', height: '50px', borderRadius: '50%', margin: '5px' }} />
                    <h3>Your uploads</h3>
                    <span className='badge text-bg-primary ms-1'>{userName}</span>
                </div>
                <Form.Group className='form-floating'>
                    <Form.Control type='text' id='find-snap' placeholder=' ' onChange={(e) => setWord(e.target.value)} />
            ...
            <Row>
                {
                    userImgList.filter(({ title }) => (title.toLocaleLowerCase().includes(word)))
